export default function QuoteHome() {
  return (
    <div className="flex items-center justify-center px-5 py-16 ">
      <div className="w-full mx-auto max-w-xl rounded-lg bg-white dark:bg-rose-400 shadow shadow-lg px-5 pt-5 pb-10 text-gray-800 dark:text-gray-50">
        <div className="w-full pt-1 text-center pb-5 mx-auto">
          <a href="#" className="block relative"></a>
        </div>
        <div className="w-full mb-5">
          <p className="text-base text-gray-600 dark:text-gray-100 text-center px-5">
            <span className=" px-1">&#10077;</span>Let Love and Kindness{" "}
            <br />
            be the motivation behind all that you do
            <span className="px-1">&#10078;</span>
          </p>
        </div>
        <div className="w-full">
          <p className="text-base text-cgray-500 dark:text-gray-100 font-bold text-center">
            1 Corinthians 16:14
          </p>
        </div>
      </div>
    </div>
  );
}
